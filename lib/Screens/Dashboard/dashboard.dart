  ...
  Future<void> StreamListener() async {
    await FirebaseFirestore.instance
      ..collection("Complains")
          ...
          .listen((event) {
        setState(() {
          Pending = event.docs.where((element) => element.data()['status'] == 'Pending').length;
          InProgress = event.docs.where((element) => element.data()['status'] == 'InProgress').length;
          Completed = event.docs.where((element) => element.data()['status'] == 'Completed').length;
          Rejected = event.docs.where((element) => element.data()['status'] == 'Rejected').length;
                              ...
                              child: SizedBox(
                                ...
                                child: StreamBuilder<QuerySnapshot?>(
                                  stream: FirebaseFirestore.instance
                                      .collection('Complains').where('username',isEqualTo: user?.email!.substring(0, user?.email!.indexOf('@')))
